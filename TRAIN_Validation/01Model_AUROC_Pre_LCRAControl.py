...
def main():
    ...
    X_ext = val_data[train_features].values  # 确保特征列名一致
    ...
    for idx, model in enumerate(models):
        ...
        print(f"\n===== 训练并进行外部验证 {original_model_names[idx]} 模型 =====")
        
        for i in range(10):
            print(f"\n迭代 {i+1}/10")
            # 内部划分仅用于确定训练子集
            X_train, _, y_train, _ = train_test_split(X_internal, y_internal, test_size=0.3, random_state=i)
            print(f"训练集: {X_train.shape}, 外部验证集: {X_ext.shape}")
            
            # 核心修改：利用特征提取器处理【训练集】与【外部验证集】
            ...
            auc_val = roc_auc_score(y_ext, y_pred_proba)
            accuracies.append(accuracy)
            aucs.append(auc_val)
            print(f"迭代 {i+1} 外部验证性能: 准确率={accuracy:.3f}, AUC={auc_val:.3f}")
    ...
    plt.tight_layout()
    plt.savefig('external_validation_roc_curves.png', dpi=300, bbox_inches='tight')
    plt.show()
