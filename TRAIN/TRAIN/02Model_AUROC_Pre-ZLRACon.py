...
def main():
    ...
    encoder, scaler, imputer, pretrain_features = pretrain_feature_encoder(
        dataset_path='Pre-RABC_RAControl96.xlsx',  # 替换为你的预训练数据路径
        epochs=50
    )
    
    # 2. 加载训练数据
    print("\n===== 加载训练数据 =====")
    train_data = pd.read_excel('02ZLRA_VS_Control.xlsx')  # 替换为你的训练数据路径
    ...
    models = [
        ...
        make_pipeline(StandardScaler(), SVC(probability=True, C=1, kernel='rbf', gamma='scale', random_state=42))
]
    ...
    plt.title('Diagnosis: Transcriptomics (RA vs HC)')
