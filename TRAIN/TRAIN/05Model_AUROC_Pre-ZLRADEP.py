...
def main():
    ...
    encoder, scaler, imputer, pretrain_features = pretrain_feature_encoder(
        dataset_path='Pre-RABC_RADEP153.xlsx',  # 更新为你的预训练数据文件
        epochs=50
    )
    
    # 加载训练数据
    print("\n加载训练数据...")
    train_data = pd.read_excel('05RA_DEPZL-FC1.5-P0.01.xlsx')  # 替换为你的训练数据路径
    ...
    plt.title('Stratification: Transcriptomics (RA vs RA_DEP)')
