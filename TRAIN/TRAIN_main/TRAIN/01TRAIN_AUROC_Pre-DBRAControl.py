...
def main():
    ...
    encoder, scaler, imputer, pretrain_features = pretrain_feature_encoder(
        dataset_path='Pre-DBRA_VS_Control.xlsx',  # 替换为你的预训练数据路径
        epochs=50
    )
    
    # 2. 加载训练数据
    print("\n===== 加载训练数据 =====")
    train_data = pd.read_excel('03DBRAVSControl42-10.xlsx')  # 替换为你的训练数据路径
    ...
    plt.title('ROC Curve for Proteomics Models')
