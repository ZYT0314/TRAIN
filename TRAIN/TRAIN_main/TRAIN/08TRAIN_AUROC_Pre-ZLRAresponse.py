...
def main():
    ...
    encoder, pretrain_features = pretrain_feature_encoder(  # 关键修改：不再返回imputer和scaler
        dataset_path='Pre-RABC_RAresponse67.xlsx',  # 28特征预训练数据
        epochs=50
    )
    
    # 2. 加载训练数据（10特征）
    print("\n===== 加载训练数据 =====")
    train_data = pd.read_excel('08ZL_RA_Resp vs RA_NonResp.xlsx')  # 10特征训练数据
    ...
    plt.title('ROC Curve for Transcriptomics Models')
