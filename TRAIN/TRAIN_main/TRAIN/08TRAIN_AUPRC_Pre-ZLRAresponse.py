...
def main():
    ...
    encoder, scaler, imputer, pretrain_features = pretrain_feature_encoder(
        dataset_path='Pre-RABC_RAresponse67.xlsx',  # 更新为你的预训练数据文件
        epochs=50
    )
    
    # 加载训练数据
    print("\n加载训练数据...")
    train_data = pd.read_excel('08ZL_RA_Resp vs RA_NonResp.xlsx')  # 更新为你的训练数据文件
    ...
    models = [
        ...
        SVC(probability=True, C=0.001, kernel='rbf', gamma='scale', random_state=42)
    ]
    ...
    plt.title('PRC Curve for Transcriptomics Models')
