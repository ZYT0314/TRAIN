...
# 保存结果到新的 Excel 文件
input_df.to_excel('step_function25-Linear.xlsx', index=False)
...
# 读取预测数据
pred_data = pd.read_excel('step_function25-Linear.xlsx')
...
# 绘制预测数据曲线（蓝色），线宽1.5
plt.plot(x_pred, y_pred, color='blue', label='Prediction', linewidth=1.5)

# **严格按指定格式修改图形参数（修正潜在格式问题）**
plt.title('True vs Predicted', fontsize=20)
plt.xlabel('', fontsize=20)  # 取消x轴字母显示，空字符串
plt.ylabel('', fontsize=20)  # 取消y轴字母显示，空字符串
plt.grid(False)  # 不显示网格线

plt.xlim(-0.55, 0.55)  # x轴范围
plt.xticks([-0.5, 0, 0.5], fontsize=28)  # x轴刻度及字体，注意中括号格式
plt.ylim(-0.55, 0.55)  # y轴范围
plt.yticks([-0.5, 0, 0.5], fontsize=28)  # y轴刻度及字体，注意中括号格式
