...
# 生成 300 个均匀分布的 x 值（更适合三次函数特征）
x_train = np.linspace(-0.5, 0.5, 300)  # 均匀分布覆盖定义域，更好捕捉三次函数非线性特征
# 计算对应的真实 y 值：使用三次函数 y = 4x³（自然在[-0.5, 0.5]范围内）
y_train = 4 * x_train ** 3  # 核心：替换为三次函数
...
# 保存结果到新的 Excel 文件
input_df.to_excel('ax3-Cat50.xlsx', index=False)  # 文件名体现三次函数+模型
...
pred_data = pd.read_excel('ax3-Cat50.xlsx')
...
plt.xticks([-0.5, 0, 0.5], fontsize=28)  # x轴刻度字体
plt.ylim(-0.55, 0.55)  # 设置 y 轴范围
plt.yticks([-0.5, 0, 0.5], fontsize=28)  # y轴刻度字体

plt.legend(fontsize=18)  # 图例字体
