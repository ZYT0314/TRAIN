...
# 保存结果到新的 Excel 文件
input_df.to_excel('abs_x21-Predicted.xlsx', index=False)
...
# 读取预测数据
pred_data = pd.read_excel('abs_x21-Predicted.xlsx')
...
plt.xticks([-0.5, 0, 0.5], fontsize=20)  # 设置 x 轴刻度
plt.ylim(-0.55, 0.55)  # 设置 y 轴范围
plt.yticks([-0.5, 0, 0.5], fontsize=20)  # 设置 y 轴刻度
