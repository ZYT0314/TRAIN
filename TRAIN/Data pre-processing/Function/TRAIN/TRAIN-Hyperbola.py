...
# 将 1D 扩展为 5D 通用数学基底，模拟高维空间的互相影响（保持框架通用性）
...
x_train_raw = np.concatenate([x_neg, x_pos])  # 合并为300个点

# 计算对应的真实y值：使用双曲线函数 y = -0.05 / x（双象限分布）
k = -0.05
y_train = k / x_train_raw  # 核心：替换为双曲线函数
...
try:
    ...
    # 保存预测结果
    input_df.to_excel('hyperbola-TRAIN40.xlsx', index=False)  # 文件名体现双曲线+模型
    ...
    pred_data = pd.read_excel('hyperbola-TRAIN40.xlsx')
    ...
    print("提示：未找到本地的 'hyperbola-True40.xlsx' 文件，跳过绘图步骤。模型已完成评估！")