...
# 生成 300 个均匀分布的 x 值（更适合周期性函数）
x_train = np.linspace(-0.5, 0.5, 300)
# 计算对应的真实 y 值：使用调整后的余弦函数（自然在[-0.5, 0.5]范围内）
y_train = 0.25 * np.cos(4 * np.pi * x_train) + 0.5 * x_train  # 核心修改：调整函数参数
...
# 保存结果到新的 Excel 文件
input_df.to_excel('Cosx-XGB50.xlsx', index=False)
...
pred_data = pd.read_excel('Cosx-XGB50.xlsx')
