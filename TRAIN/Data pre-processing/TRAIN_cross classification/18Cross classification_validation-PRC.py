...
OUTPUT_FIGURE = "External_Validation_Clinical_Pairwise_PRC.png"
N_REPEATS = 10
TEST_SIZE = 0.30
BASE_SEED = 42
...
def run_single_task(task_config, development_sheet, validation_sheet):
    ...
    # 6. 重复10次：DZCH仅使用70%训练，西南医院始终完整验证
    accuracies = []
    auprcs = []
    precisions_list = []
    recalls_list = []
    ...
    for i in range(N_REPEATS):
        ...
        print(f"{task_name}迭代 {i + 1} 外部验证性能: 准确率={accuracy:.3f}, AUPRC={auprc:.3f}")
    ...
    for precision, recall in zip(precisions_list, recalls_list):
        ...
        interp_precision = np.interp(mean_recall, recall[idx_sort], precision[idx_sort])
...
def main():
    ...
    # 4. 打印三个任务最终外部验证结果
    print("\n" + "=" * 100)
    print("===== TRAIN三个疾病对西南医院独立外部验证AUPRC结果 =====")
    print("=" * 100)
