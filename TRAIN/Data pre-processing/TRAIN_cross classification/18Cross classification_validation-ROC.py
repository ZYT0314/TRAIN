...
VALIDATION_FILE = "西南医院疾病分类验证.xlsx"
MODEL_PATH = "tabpfn-v3-classifier-v3_20260417_binary.ckpt"
OUTPUT_FIGURE = "External_Validation_Clinical_Pairwise_ROC.png"
N_REPEATS = 10
TEST_SIZE = 0.30
BASE_SEED = 42

# 两个Excel前三个Sheet按相同顺序分别对应三个任务
TASK_CONFIGS = [
...
# ===================== 9. 单个疾病对独立外部验证 =====================
def run_single_task(task_config, development_sheet, validation_sheet):
    task_name = task_config["name"]
    pretrain_file = task_config["pretrain_file"]

    print("\n" + "=" * 100)
    print(f"开始独立外部验证任务：{task_name}")
    print(f"DZCH开发Sheet：{development_sheet}")
    print(f"西南医院验证Sheet：{validation_sheet}")
    print(f"预训练文件：{pretrain_file}")
    print("=" * 100)

    # 每个任务重新固定随机种子，三个任务彼此独立
    set_random_seed(BASE_SEED)
    ...
    # 2. 加载当前任务DZCH开发数据
    print(f"\n===== {task_name}：加载DZCH开发数据 =====")
    train_data = pd.read_excel(DEVELOPMENT_FILE, sheet_name=development_sheet)
    X_internal, y_internal, train_features = prepare_dataset(train_data, task_name)

    print(f"DZCH数据维度: {X_internal.shape}")
    print(f"DZCH N={(y_internal == 0).sum()}, T={(y_internal == 1).sum()}")

    # 3. 加载当前任务西南医院独立外部验证数据
    print(f"\n===== {task_name}：加载西南医院独立外部验证数据 =====")
    val_data = pd.read_excel(VALIDATION_FILE, sheet_name=validation_sheet)

    if "group" not in val_data.columns:
        raise ValueError(f"{task_name}西南医院验证Sheet缺少group列")

    missing_features = [col for col in train_features if col not in val_data.columns]
    if missing_features:
        raise ValueError(f"{task_name}西南医院验证数据缺少开发集特征：{missing_features}")

    val_group = val_data["group"].astype(str).str.strip().str.upper()
    y_ext = val_group.map({"N": 0, "T": 1})

    if y_ext.isna().any():
        invalid_values = val_data.loc[y_ext.isna(), "group"].drop_duplicates().tolist()
        raise ValueError(f"{task_name}西南医院验证数据存在非N/T标签：{invalid_values}")

    if y_ext.nunique() != 2:
        raise ValueError(f"{task_name}西南医院验证数据必须同时包含N和T两个类别")

    y_ext = y_ext.astype(int).values
    X_ext = val_data[train_features].values

    print(f"西南医院外部验证数据维度: {X_ext.shape}")
    print(f"西南医院 N={(y_ext == 0).sum()}, T={(y_ext == 1).sum()}")
    ...
    # 6. 重复10次：每轮仅从DZCH抽取70%训练，西南医院完整验证
    accuracies = []
    aucs = []
    fprs = []
    tprs = []
    ...
    for i in range(N_REPEATS):
        ...
        # 与原代码完全一致：30%仅被舍弃，不参与训练和评价
        X_train, _, y_train, _ = train_test_split(X_internal, y_internal, test_size=TEST_SIZE, random_state=i)
        print(f"DZCH训练集: {X_train.shape}, 西南医院外部验证集: {X_ext.shape}")

        # 仅用当前DZCH训练子集与完整西南医院验证集完成特征处理
        ...
        auc_value = roc_auc_score(y_ext, y_pred_proba)
        fpr, tpr, _ = roc_curve(y_ext, y_pred_proba)
        ...
        print(f"{task_name}迭代 {i + 1} 外部验证性能: 准确率={accuracy:.3f}, AUROC={auc_value:.3f}")
...
def main():
    # 1. 检查全部文件
    required_files = [DEVELOPMENT_FILE, VALIDATION_FILE, MODEL_PATH] + [task["pretrain_file"] for task in TASK_CONFIGS]

    for file_path in required_files:
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"未找到文件：{file_path}")

    # 2. 获取两个Excel中的Sheet顺序
    development_sheets = pd.ExcelFile(DEVELOPMENT_FILE).sheet_names
    validation_sheets = pd.ExcelFile(VALIDATION_FILE).sheet_names

    if len(development_sheets) < 3:
        raise ValueError(f"{DEVELOPMENT_FILE}至少需要3个Sheet，当前只有{len(development_sheets)}个：{development_sheets}")

    if len(validation_sheets) < 3:
        raise ValueError(f"{VALIDATION_FILE}至少需要3个Sheet，当前只有{len(validation_sheets)}个：{validation_sheets}")

    development_sheets = development_sheets[:3]
    validation_sheets = validation_sheets[:3]

    print("\n===== 三个独立外部验证任务配置 =====")

    for i, task in enumerate(TASK_CONFIGS):
        print(f"{i + 1}. {task['name']} → DZCH Sheet: {development_sheets[i]} → 西南医院 Sheet: {validation_sheets[i]} → Pretrain: {task['pretrain_file']}")

    # 3. 三个任务依次完全独立运行
    all_results = []

    for i, task_config in enumerate(TASK_CONFIGS):
        result = run_single_task(task_config, development_sheets[i], validation_sheets[i])
        all_results.append(result)

    # 4. 打印三个任务最终外部验证结果
    print("\n" + "=" * 100)
    print("===== TRAIN三个疾病对西南医院独立外部验证AUROC结果 =====")
    print("=" * 100)

    for result in all_results:
        print(f"{result['name']}:")
        print(f"  DZCH Sheet: {result['development_sheet']}")
        print(f"  西南医院 Sheet: {result['validation_sheet']}")
    ...
    for result in all_results:
        ...
        plt.plot(result["mean_fpr"], result["mean_tpr"], lw=2, label=f"{result['name']} AUROC: {result['mean_auc']:.3f} (±{result['std_auc']:.3f})", color=result["color"])
        plt.fill_between(result["mean_fpr"], lower_tpr, upper_tpr, alpha=0.08, color=result["color"])

    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("Validation: Clinical Pairwise Classification")
