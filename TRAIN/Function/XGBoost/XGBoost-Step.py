...
# 保存结果到新的Excel文件
input_df.to_excel('step_function25-XGB.xlsx', index=False)
...
# 读取预测数据
pred_data = pd.read_excel('step_function25-XGB.xlsx')
...
# 绘制真实数据曲线图（橙色），添加label='True'，线宽设为3
...
plt.title('True vs Predicted', fontsize=20)  # 标题字体设置为20
plt.grid(False)  # 不显示网格线

plt.xlim(-0.55, 0.55)  # 设置x轴范围
plt.xticks([-0.5, 0, 0.5], fontsize=28)  # x轴刻度字体设置为28
plt.ylim(-0.55, 0.55)  # 设置y轴范围
plt.yticks([-0.5, 0, 0.5], fontsize=28)  # y轴刻度字体设置为28

# 重新调整图例，确保颜色和标签对应正确
handles, labels = plt.gca().get_legend_handles_labels()
by_label = dict(zip(labels, handles))
plt.legend(by_label.values(), by_label.keys(), fontsize=18)  # 图例字体设置为18
