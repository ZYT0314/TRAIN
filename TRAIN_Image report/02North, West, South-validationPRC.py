...
OUTPUT_FIGURE = "Phenotype: Imaging (IP vs DP)_PRC_External.png"
N_REPEATS = 10
TEST_SIZE = 0.30
BASE_SEED = 42
EXTERNAL_ENCODER_SEED_OFFSET = 10000
EXTERNAL_MODEL_SEED_OFFSET = 20000
...
class UnifiedFeatureExtractor:
    ...
    def transform(self, X_train, X_test):
        ...
        matched_imputer = SimpleImputer(strategy="mean")
        matched_scaler = StandardScaler()
        unmatched_imputer = SimpleImputer(strategy="mean")
        unmatched_scaler = StandardScaler()
        ...
        if X_train_matched.size > 0:
            if self.pretrain_imputer.statistics_.shape[0] != X_train_matched.shape[1]:
                ...
                X_train_matched = matched_imputer.fit_transform(X_train_matched)
                X_train_matched = matched_scaler.fit_transform(X_train_matched)
                X_test_matched = matched_imputer.transform(X_test_matched)
                X_test_matched = matched_scaler.transform(X_test_matched)
            else:
        ...
        if X_train_unmatched.size > 0:
            X_train_unmatched = unmatched_imputer.fit_transform(X_train_unmatched)
            X_train_unmatched = unmatched_scaler.fit_transform(X_train_unmatched)
            X_test_unmatched = unmatched_imputer.transform(X_test_unmatched)
            X_test_unmatched = unmatched_scaler.transform(X_test_unmatched)
...
def pretrain_feature_encoder(dataset_path, mask_ratio=0.2, epochs=50, batch_size=64, lr=1e-3):
    ...
    if "group" not in pretrain_data.columns:
        raise ValueError(f"预训练文件缺少group列：{dataset_path}")

    pretrain_feature_names = [col for col in pretrain_data.columns if col != "group"]
    X_pretrain = pretrain_data[pretrain_feature_names].values
    ...
    for epoch in range(epochs):
        total_loss = 0
        dataloader = DataLoader(TensorDataset(torch.FloatTensor(X_pretrain)), batch_size=batch_size, shuffle=True, generator=generator)
...
def prepare_dataset(data, train_features, dataset_name):
    ...
    if y.nunique() < 2:
        raise ValueError(f"{dataset_name}只有一个类别，不能计算AUPRC")

    X = data[train_features].values
    return X, y.astype(int).values

# ===================== 9. 单个外部验证集独立运行 =====================
def evaluate_external_validation(cohort_name, cohort_color, X_internal, y_internal, X_external, y_external, feature_extractor, model_path, n_repeats, test_size, mean_recall):
    accuracies = []
    auprcs = []
    precisions_list = []
    recalls_list = []
    ...
    for i in range(n_repeats):
        split_seed = i
        encoder_seed = BASE_SEED + EXTERNAL_ENCODER_SEED_OFFSET + i
        model_seed = BASE_SEED + EXTERNAL_MODEL_SEED_OFFSET + i

        print(f"\n{cohort_name}迭代 {i + 1}/{n_repeats}")
        print(f"数据划分种子={split_seed}，外部编码器种子={encoder_seed}，外部模型种子={model_seed}")
        ...
        print(f"{cohort_name}迭代 {i + 1} 性能: 准确率={accuracy:.3f}, AUPRC={auprc:.3f}")
...
def main():
    ...
    print(f"每次迭代使用DZCH开发集的{(1 - TEST_SIZE) * 100:.0f}%训练，剩余{TEST_SIZE * 100:.0f}%不参与训练、评价或绘图")
    ...
    for dataset in validation_datasets:
        external_result = evaluate_external_validation(dataset["name"], dataset["color"], X_internal, y_internal, dataset["X"], dataset["y"], feature_extractor, MODEL_PATH, N_REPEATS, TEST_SIZE, mean_recall)
        all_results.append(external_result)

    print("\n===== TRAIN模型三个独立外部验证集AUPRC结果 =====")
    ...
    for result in all_results:
        ...
        plt.plot(mean_recall, result["mean_precision"], lw=2, label=f"{result['name']} AUPRC: {result['mean_auprc']:.3f} (±{result['std_auprc']:.3f})", color=result["color"])
        plt.fill_between(mean_recall, lower_precision, upper_precision, alpha=0.08, color=result["color"])
